...
public struct MValuationAssetPosition: Hashable & AllocBase {
    ...
    public enum CodingKeys: String, CodingKey, CaseIterable {
        case snapshotID = "valuationAssetPositionSnapshotID"
        case accountID = "valuationAssetPositionAccountID"
        case assetID = "valuationAssetPositionAssetID"
        case totalBasis
        case marketValue
    }

    public static var schema: AllocSchema { .allocValuationAssetPosition }
    ...
    public init(from row: Row) throws {
        guard let snapshotID_ = MValuationAssetPosition.getStr(row, CodingKeys.snapshotID.rawValue)
        else { throw AllocDataError.invalidPrimaryKey(CodingKeys.snapshotID.rawValue) }
        snapshotID = snapshotID_

        guard let accountID_ = MValuationAssetPosition.getStr(row, CodingKeys.accountID.rawValue)
        else { throw AllocDataError.invalidPrimaryKey(CodingKeys.accountID.rawValue) }
        accountID = accountID_

        guard let assetID_ = MValuationAssetPosition.getStr(row, CodingKeys.assetID.rawValue)
        else { throw AllocDataError.invalidPrimaryKey(CodingKeys.assetID.rawValue) }
        assetID = assetID_

        totalBasis = MValuationAssetPosition.getDouble(row, CodingKeys.totalBasis.rawValue) ?? 0
        marketValue = MValuationAssetPosition.getDouble(row, CodingKeys.marketValue.rawValue) ?? 0
    }

    public mutating func update(from row: Row) throws {
        if let val = MValuationAssetPosition.getDouble(row, CodingKeys.totalBasis.rawValue) { totalBasis = val }
        if let val = MValuationAssetPosition.getDouble(row, CodingKeys.marketValue.rawValue) { marketValue = val }
    }

    public var primaryKey: AllocKey {
        MValuationAssetPosition.makePrimaryKey(snapshotID: snapshotID, accountID: accountID, assetID: assetID)
    }
    ...
    public static func getPrimaryKey(_ row: Row) throws -> AllocKey {
        ...
        else { throw AllocDataError.invalidPrimaryKey("AssetPosition") }
    ...
    public static func decode(_ rawRows: [RawRow], rejectedRows: inout [Row]) throws -> [Row] {
        let ck = MValuationAssetPosition.CodingKeys.self
...
extension MValuationAssetPosition: CustomStringConvertible {
